...
var __razdalja_7_useless_var__ unsafe.Pointer = nil

const (
	KeyRazdalja_7 = "razdalja_7"
)


type vectordistance7Args struct {
    p uintptr
    a uintptr
    b uintptr
    len int32
...
/*var (
    vectordistance7Args = vectordistance7Args{}
...
func VectorDistance7(grid, block cuda.Dim3, p uintptr, a uintptr, b uintptr, len int32) error {
	err := autoloadLib_razdalja_7()
	if err != nil {
		return err
	}
	kern, err := getKernel("razdalja_7", "vectorDistance7")
	if err != nil {
		return err
	}
	
	params := vectordistance7Args{
	    p: p,
	    a: a,
	    b: b,
	    len: len,
...
func VectorDistance7Ex(grid, block cuda.Dim3, sharedMem uint64, stream *cuda.Stream, p uintptr, a uintptr, b uintptr, len int32) error {
	err := autoloadLib_razdalja_7()
	if err != nil {
		return err
	}
...
var loaded_razdalja_7 = false


func autoloadLib_razdalja_7() error {
	if loaded_razdalja_7 {
		return nil
	}
	err := InitLibrary([]byte(Razdalja_7_ptxCode), "razdalja_7")
	if err != nil {
		return err
	}
	loaded_razdalja_7 = true
	return nil
}

const Razdalja_7_ptxCode = `//
...
.address_size 64
	...
	// .globl	vectorDistance7
.extern .shared .align 16 .b8 part[];

.visible .entry vectorDistance7(
	.param .u64 vectorDistance7_param_0,
	.param .u64 vectorDistance7_param_1,
	.param .u64 vectorDistance7_param_2,
	.param .u32 vectorDistance7_param_3
)
{
	.reg .pred 	%p<10>;
	.reg .f32 	%f<15>;
	...
	ld.param.u64 	%rd4, [vectorDistance7_param_0];
	ld.param.u64 	%rd5, [vectorDistance7_param_1];
	ld.param.u64 	%rd6, [vectorDistance7_param_2];
	ld.param.u32 	%r14, [vectorDistance7_param_3];
	mov.u32 	%r1, %tid.x;
