...
// 		srun --partition=gpu --gpus=1 go run urejanje-nl.go
...
func main() {
	...
	bytesLocalMemory := 2 * uint64(*numThreadsPtr) * bytesInt
	...
	for k := 4 * int32(blockSize.X); k <= int32(*tableSizePtr); k <<= 1 {                               // k = 4 * blockSize ... tableLength
		...
		cudago.BitonicSortFinishEx(gridSize, blockSize, bytesLocalMemory, nil, da.Ptr, int32(*tableSizePtr), k) //   j = 2 * blockSize.x ... 1
	}
