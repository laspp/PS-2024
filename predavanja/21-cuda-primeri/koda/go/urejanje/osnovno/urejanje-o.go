...
// 		srun --partition=gpu --gpus=1 go run urejanje-o.go
...
func main() {
	...
	numBlocks := (*tableSizePtr-1) / *numThreadsPtr + 1
	...
	for k := int32(2); k <= int32(*tableSizePtr); k <<= 1 {
		for j := k / 2; j > 0; j >>= 1 {
			err = cudago.BitonicSortO(gridSize, blockSize, da.Ptr, int32(*tableSizePtr), k, j)
			if err != nil {
				panic(err)
			}
		}
